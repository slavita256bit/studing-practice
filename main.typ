...
#show outline.entry: it => {
  show linebreak: []
  let clean_body = {
    // Убираем статус (Обязательное) и (Справочное)
    show "(Обязательное)": ""
    show "(Справочное)": ""

    // Превращаем жирный текст заголовка в обычный для содержания
  ...
  if state("appendixes", false).at(it.element.location()) {
    // Принудительно делаем всю строчку приложения в содержании не жирной
...
В таблице @data-structures-products представлена иерархия структур данных, описывающих товары. Структуры расположены сверху и объединяют свои внутренние поля, находящиеся на уровнях ниже. Независимые структуры и перечисления вынесены в таблицы @data-category и @data-type. Общая схема работы программы (функция main) приведена в
приложении #ref(<main-scheme>, supplement: none).
...
Функция добавления нового товара в бинарное дерево поиска игнорируя дубликаты по идентификатору. Схема алгоритма представлена в
приложении~#ref(<tree_add_element-scheme>, supplement: none)).
...
Полный исходный код разработанного консольного приложения представлен
в приложении #ref(<code-appendix>, supplement: none).
...
#let appendix-heading(status, level: 1, body) = {
  heading(level: level, supplement: [Приложение])[
    #text(weight: "regular")[(#status)]\
    #v(1em)
    *#body*
  ]
}
...
#[
  ...
  #place(top + left, dx: -5mm, dy: -20mm, block(
    ...
    table(
      ...
      // Строки 15-17
      [БГУИР.6-05-0611-05.114 ПЗ], [Отчет по учебной (ознакомительной) практике], [#context [ #counter(page).final().first() с. ]],
      [], [], [],
      [], [], [],
